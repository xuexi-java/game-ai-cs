# 运维部署命令

> 📝 **日志说明**: 系统日志为 JSON 单行格式，stdout/stderr 分流，支持 ELK/Loki。详见 [日志配置说明](./日志配置说明.md)

## 1. 启动服务

### 1.1 启动核心服务（后端 + 数据库 + 前端）

```bash
# 启动核心服务
docker-compose up -d

# 查看服务状态
docker-compose ps

# 查看日志
docker-compose logs -f backend
```

### 1.2 启动全部服务（包含监控）

```bash
# 启动核心服务 + 监控服务（Prometheus + Grafana + Loki）
docker-compose --profile monitoring up -d

# 查看所有服务状态
docker-compose --profile monitoring ps

# 查看监控日志
docker-compose --profile monitoring logs -f prometheus grafana
```

---

## 2. 停止服务

### 2.1 停止核心服务

```bash
docker-compose down
```

### 2.2 停止全部服务（包含监控）

```bash
docker-compose --profile monitoring down
```

---

## 3. 重启服务

### 3.1 重启后端服务

```bash
docker-compose restart backend
```

### 3.2 重启监控服务

```bash
docker-compose restart prometheus grafana
```

---

## 4. 日志管理

### 4.1 查看日志

#### Docker 环境

```bash
# 实时查看后端日志
docker-compose logs -f backend

# 查看最近 100 行
docker-compose logs --tail=100 backend

# 查看 Prometheus 日志
docker-compose logs -f prometheus

# 查看 Grafana 日志
docker-compose logs -f grafana
```

#### PM2 环境

```bash
# 查看所有日志
pm2 logs game-ai-backend

# 只查看 stdout（INFO/WARN）
pm2 logs game-ai-backend --out

# 只查看 stderr（ERROR）
pm2 logs game-ai-backend --err

# 清空日志
pm2 flush
```

### 4.2 日志文件位置

```bash
# Docker 环境（容器内）
/app/logs/access.log    # stdout 日志（INFO/WARN）
/app/logs/error.log     # stderr 日志（ERROR）
/app/logs/cleanup.log   # 清理日志

# PM2 环境（宿主机）
backend/logs/access.log    # stdout 日志
backend/logs/error.log     # stderr 日志
backend/logs/cleanup.log   # 清理日志
```

### 4.3 日志查询

#### 按 traceId 查询完整请求链路

```bash
# Docker 环境
docker exec game-ai-cs-backend grep "a1b2c3d4-e5f6-7890" /app/logs/access.log /app/logs/error.log

# PM2 环境
grep "a1b2c3d4-e5f6-7890" backend/logs/access.log backend/logs/error.log
```

#### 按日志级别查询

```bash
# 查询所有 ERROR 日志
grep '"level":"ERROR"' backend/logs/error.log | jq .

# 查询所有 WARN 日志
grep '"level":"WARN"' backend/logs/access.log | jq .
```

#### 按业务操作查询

```bash
# 查询工单创建日志
grep '"action":"ticket_created"' backend/logs/access.log | jq .

# 查询会话转人工日志
grep '"action":"session_transferred"' backend/logs/access.log | jq .
```

### 4.4 日志轮转与清理

#### 自动轮转（按大小）

日志文件达到 100MB 自动轮转，保留最近 10 个文件。

```bash
# PM2 自动轮转配置（已配置）
# max_size: 100M
# max_files: 10

# Docker 自动轮转配置（已配置）
# max-size: 100m
# max-file: 10
```

#### 定期清理（按时间）

自动删除 3 个月前的日志文件。

**Linux/macOS 环境：**

```bash
# 1. 设置定时任务（每月 1 号凌晨 2:00 执行）
cd /path/to/project
chmod +x scripts/setup-log-cleanup-cron.sh
./scripts/setup-log-cleanup-cron.sh

# 2. 查看定时任务
crontab -l

# 3. 手动执行清理
./scripts/clean-logs.sh

# 4. 查看清理日志
cat backend/logs/cleanup.log
```

**Windows 环境：**

```powershell
# 1. 以管理员身份运行 PowerShell
# 2. 设置定时任务（每月 1 号凌晨 2:00 执行）
cd C:\path\to\project
.\scripts\setup-log-cleanup-task.ps1

# 3. 查看定时任务
Get-ScheduledTask -TaskName "GameAI-LogCleanup"

# 4. 手动执行清理
powershell -ExecutionPolicy Bypass -File .\scripts\clean-logs.ps1

# 5. 查看清理日志
Get-Content backend\logs\cleanup.log
```

#### 清理策略

| 操作 | 触发条件 | 执行方式 | 说明 |
|------|----------|----------|------|
| 日志轮转 | 文件大小 > 100MB | PM2/Docker 自动 | 创建 .log.1, .log.2 等 |
| 日志压缩 | 文件修改时间 > 7 天 | 清理脚本自动 | 压缩为 .log.*.gz |
| 日志删除 | 文件修改时间 > 90 天 | 清理脚本自动 | 永久删除 |

### 4.5 日志监控告警

建议监控以下指标：

```bash
# 1. 日志目录大小
du -sh backend/logs

# 2. ERROR 日志数量（最近 1 小时）
grep '"level":"ERROR"' backend/logs/error.log | grep "$(date -u +%Y-%m-%dT%H)" | wc -l

# 3. 慢请求数量（最近 1 小时）
grep '"costMs":[0-9]\{4,\}' backend/logs/access.log | grep "$(date -u +%Y-%m-%dT%H)" | wc -l

# 4. 工单自动关闭数量（今天）
grep '"action":"ticket_closed"' backend/logs/access.log | grep "$(date +%Y-%m-%d)" | wc -l

# 5. 定时任务执行记录（最近 24 小时）
grep '定时任务完成' backend/logs/access.log | grep "$(date -u +%Y-%m-%d)" | jq .
```

---

## 5. 数据库操作

### 5.1 进入 PostgreSQL

```bash
docker exec -it game-ai-cs-postgres psql -U postgres -d game_ai_cs
```

### 5.2 数据库备份

```bash
docker exec game-ai-cs-postgres pg_dump -U postgres game_ai_cs > backup.sql
```

### 5.3 数据库恢复

```bash
cat backup.sql | docker exec -i game-ai-cs-postgres psql -U postgres -d game_ai_cs
```

---

## 6. Redis 操作

### 6.1 进入 Redis CLI

```bash
docker exec -it game-ai-cs-redis redis-cli
```

### 6.2 查看 Redis 键

```bash
docker exec -it game-ai-cs-redis redis-cli KEYS "*"
```

### 6.3 清空 Redis

```bash
docker exec -it game-ai-cs-redis redis-cli FLUSHALL
```

---

## 7. 访问地址

### 7.1 应用访问

- 后端 API: http://localhost:21101/api/v1
- 管理端文档: http://localhost:21101/api/v1/docs/admin
- 玩家端文档: http://localhost:21101/api/v1/docs/player
- 管理端前端: http://localhost:20101
- 玩家端前端: http://localhost:20102

### 7.2 监控访问

- Prometheus: http://localhost:23101
- Grafana: http://localhost:23103
- Loki: http://localhost:23102
  - 默认用户名: `admin`
  - 默认密码: `admin`
- Metrics 端点: http://localhost:21101/api/v1/metrics

---

## 8. 健康检查

### 8.1 检查后端健康

```bash
curl http://localhost:21101/api/v1/metrics
```

### 8.2 检查 Prometheus 目标状态

访问: http://localhost:23101/targets

应显示 `game-ai-backend` 状态为 `UP`

### 8.3 检查 Grafana Dashboard

访问: http://localhost:23103/dashboards

应看到 `Game AI Backend - Overview` Dashboard

---

## 9. 清理数据

### 9.1 清理所有容器和数据卷

```bash
# 停止并删除所有容器和数据卷（包含监控服务）
docker-compose --profile monitoring down -v
```

### 9.2 清理 Docker 镜像

```bash
docker image prune -a
```

---

## 10. 故障排查

### 10.1 后端无法连接数据库

```bash
# 检查数据库容器状态
docker-compose ps postgres

# 查看数据库日志
docker-compose logs postgres

# 重启数据库
docker-compose restart postgres
```

### 10.2 Prometheus 无法抓取指标

```bash
# 检查后端是否启动
docker-compose ps backend

# 检查 metrics 端点
curl http://localhost:21101/api/v1/metrics

# 检查网络连接
docker network inspect game-ai-cs-network
```

### 10.3 Grafana 无数据

```bash
# 检查 Prometheus 数据源
# 访问 Grafana -> Configuration -> Data Sources

# 检查 Prometheus 是否有数据
# 访问 http://localhost:23101/graph
# 执行查询: sum(queue_length)
```

---

## 11. 更新部署

### 11.1 更新后端代码

```bash
# 停止后端
docker-compose stop backend

# 重新构建
docker-compose build backend

# 启动后端
docker-compose up -d backend
```

### 11.2 更新监控配置

```bash
# 修改配置文件后重启
docker-compose restart prometheus grafana
```

---

## 12. 生产环境注意事项

### 12.1 环境变量

确保在 `.env` 文件中配置：

```bash
# 数据库
DATABASE_URL=postgresql://postgres:postgres@postgres:5432/game_ai_cs

# JWT
JWT_SECRET=your-production-secret-key

# Redis
REDIS_HOST=redis
REDIS_PORT=6379

# 前端地址
FRONTEND_URL=https://your-domain.com
```

### 12.2 数据持久化

数据卷位置：
- PostgreSQL: `postgres_data`
- Redis: `redis_data`
- Prometheus: `prometheus_data`
- Grafana: `grafana_data`

### 12.3 日志管理

生产环境必须配置：

```bash
# 1. 设置日志级别
LOG_LEVEL=INFO

# 2. 配置日志轮转（已自动配置）
# PM2: max_size=100M, max_files=10
# Docker: max-size=100m, max-file=10

# 3. 配置定期清理（每月 1 号凌晨 2:00）
# Linux/macOS: 运行 scripts/setup-log-cleanup-cron.sh
# Windows: 运行 scripts/setup-log-cleanup-task.ps1

# 4. 监控日志目录大小
du -sh backend/logs

# 5. 定期检查清理日志
cat backend/logs/cleanup.log
```

### 12.4 备份策略

建议定期备份：
- 数据库（每天）
- Redis（每小时）
- Grafana Dashboard 配置（每次修改后）
- 日志文件（保留 3 个月，自动清理）
