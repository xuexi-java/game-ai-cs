...
export class BaiduTranslationProvider implements TranslationProvider {
  ...
  constructor(
    ...
    this.logger.debug(
      `  Raw App ID from env: "${rawAppId}" (length: ${rawAppId.length})`,
    );
    this.logger.debug(
      `  Raw Secret from env: "${rawSecret ? '*'.repeat(rawSecret.length) : 'missing'}" (length: ${rawSecret?.length || 0})`,
    );
    ...
    this.logger.debug(
      `  Cleaned App ID: "${this.appId}" (length: ${this.appId.length})`,
    );
    this.logger.debug(
      `  Cleaned Secret: "${this.secret ? '*'.repeat(this.secret.length) : 'missing'}" (length: ${this.secret?.length || 0})`,
    );
    ...
    if (this.secret && /^\*+$/.test(this.secret)) {
      ...
      this.logger.error(
        `     改为 BAIDU_TRANSLATE_SECRET=H1dETwWWqk45uN2DzGxK`,
      );
    ...
    if (!this.appId || !this.secret) {
      ...
      this.logger.error(
        `  App ID: "${this.appId}" (length: ${this.appId.length})`,
      );
      this.logger.error(
        `  Secret: "${this.secret ? '*'.repeat(this.secret.length) : 'missing'}" (length: ${this.secret?.length || 0})`,
      );
      ...
      if (this.secret.length !== 20) {
        this.logger.warn(
          `⚠️  Warning: Secret length is ${this.secret.length}, expected 20. This may cause signature errors.`,
        );
      } else {
      ...
      if (!/^[a-zA-Z0-9]+$/.test(this.secret)) {
        this.logger.warn(
          `⚠️  Warning: Secret contains non-alphanumeric characters. This may cause signature errors.`,
        );
        // 输出Secret中每个字符的详细信息
        ...
        this.logger.warn(
          `  Invalid characters found: ${invalidChars.map((c) => `'${c}' (code: ${c.charCodeAt(0)})`).join(', ')}`,
        );
      } else {
      ...
      if (this.secret.length >= 6) {
        ...
        if (actualStart === expectedStart && actualEnd === expectedEnd) {
          this.logger.debug(
            `  ✅ Secret value appears to be correct (verified by prefix/suffix)`,
          );
        } else {
          this.logger.warn(`⚠️  Warning: Secret value may be incorrect`);
          this.logger.warn(
            `  Expected start: "${expectedStart}", actual: "${actualStart}"`,
          );
          this.logger.warn(
            `  Expected end: "${expectedEnd}", actual: "${actualEnd}"`,
          );
        }
      }
    }
  ...
  private sign(q: string, salt: string): string {
    ...
    this.logger.debug(
      `  appId: "${this.appId}" (length: ${this.appId.length})`,
    );
    this.logger.debug(
      `  query: "${query}" (length: ${query.length}, bytes: ${Buffer.from(query, 'utf8').length})`,
    );
    this.logger.debug(`  salt: "${salt}"`);
    this.logger.debug(
      `  secret: "${'*'.repeat(this.secret.length)}" (length: ${this.secret.length})`,
    );
    // 显示完整的签名字符串（对于短文本）或预览（对于长文本）
    if (str.length <= 100) {
      this.logger.debug(
        `  sign string: "${this.appId}${query}${salt}${'*'.repeat(this.secret.length)}"`,
      );
    } else {
      this.logger.debug(
        `  sign string preview: "${this.appId}${query.substring(0, 20)}...${query.substring(query.length - 20)}${salt}${'*'.repeat(this.secret.length)}"`,
      );
    }
  ...
  async translate(
    ...
    try {
      ...
      if (data.error_code) {
        ...
        switch (data.error_code) {
          case 54001:
            ...
            if (this.secret.length >= 6) {
              this.logger.error(
                `  Secret preview (for verification): "${this.secret.substring(0, 3)}...${this.secret.substring(this.secret.length - 3)}"`,
              );
            } else {
              this.logger.error(`  Secret preview: "${this.secret}"`);
            }
            this.logger.error(
              `  Sign string used: "${this.appId}${text.substring(0, Math.min(20, text.length))}${text.length > 20 ? '...' : ''}${salt}${'*'.repeat(this.secret.length)}"`,
            );
            this.logger.error(
              `  💡 提示: 请使用 test-baidu-sign.js 脚本验证 Secret 是否正确`,
            );
            break;
          case 54003:
      ...
      if (
        ...
        this.logger.warn(
          '💡 提示：请检查网络连接或配置代理（HTTP_PROXY/HTTPS_PROXY）',
        );
        return {
